...
async function signIn(identifier, password) {
  ...
  const token = rawResponse.replace(/^"|"$/g, "").trim();
...
function donutChart(container, parts) {
  ...
  // Center label: use the first visible part for consistency with the original design
...
async function loadProfile() {
  const tokenClaims = parseJwt(getToken());
  const userIdFromToken = Number(tokenClaims?.sub);
  ...
  getElement("hello").textContent = `Welcome back, ${currentUser.login}`;
  getElement("i-login").textContent = currentUser.login;
  getElement("i-id").textContent = currentUser.id;
  getElement("i-name").textContent =
    [currentUser.attrs?.firstName, currentUser.attrs?.lastName].filter(Boolean).join(" ") || "—";
  getElement("i-email").textContent = currentUser.attrs?.email || "—";
  getElement("i-campus").textContent = currentUser.attrs?.campus || "Reboot01";

  // XP transactions and chart
  const xpTransactions = userDetails.transaction;
  const totalXp = xpTransactions.reduce((sum, transaction) => sum + transaction.amount, 0);

  getElement("i-xp").textContent = formatXp(totalXp);
  getElement("i-xp-sub").textContent = `${xpTransactions.length} transactions`;
  ...
  lineChart(getElement("chart-xp"), xpPoints);

  // Top projects by XP (only actual projects)
  const projectXpMap = {};
  ...
  barChart(getElement("chart-projects"), topProjects);

  // Pass/fail statistics
  const gradedProgress = userDetails.progress.filter((progress) => progress.grade !== null);
  const passed = gradedProgress.filter((progress) => progress.grade >= 1).length;
  const failed = gradedProgress.length - passed;

  getElement("i-pass").textContent = passed;
  getElement("i-fail").textContent = failed;

  donutChart(getElement("chart-ratio"), [
  ...
  getElement("i-ratio").textContent = auditRatio.toFixed(2);
  getElement("i-done").textContent = formatXp(auditsDone);
  getElement("i-received").textContent = formatXp(auditsReceived);

  donutChart(getElement("chart-audit"), [
  ...
  // Recent results list
  const recentResultsList = getElement("recent");
  recentResultsList.replaceChildren();

  userDetails.result.forEach((result) => {
...
function showProfile() {
  getElement("login-view").style.display = "none";
  getElement("profile-view").style.visibility = "visible";
}

function showLogin() {
  getElement("profile-view").style.visibility = "hidden";
  getElement("login-view").style.display = "";
}
function hideLogin(message) {
  getElement("login-view").style.display = "none";
}
...
getElement("login-form").addEventListener("submit", async (event) => {
  ...
  try {
    const identifier = getElement("identifier").value.trim();
